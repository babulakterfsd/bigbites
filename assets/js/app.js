...
const seeFoodDetails = (idMeal) => {
  ...
  fetchData(link, (data) => {
    ...
    foodDetailsContainer.querySelector(".modal-footer").innerHTML = `
      <button type="button" class="btn btn-outline-danger shadow-none" data-bs-dismiss="modal">
        Close
      </button>
      <button onclick="addToCart(${idMeal})" class="btn btn-outline-success shadow-none">
        Add to cart
      </button>
...
const cardFooter = (data, type) => {
  ...
  <div class="d-flex flex-column flex-md-row justify-content-between">
    <button  type="button" class="btn btn-outline-success shadow-none" data-bs-toggle="modal" data-bs-target="#food-details" onclick="seeFoodDetails(${data.idMeal})">
      See details
    </button>
    <div onclick="addToCart(${data.idMeal})" class="btn btn-outline-success shadow-none">Add to cart</div>
  </div>
  `;
  } else {
    return `
  <div class="d-flex justify-content-center">
    <button  type="button" class="btn btn-outline-success shadow-none" onclick="seeFoodCatagories('${data.strCategory}')">
      See Catagories
    </button>
  </div>
  `;
  ...
  if (searchData) {
    ...
    fetchData(link, (data) => {
      ...
      if (allItems != null) {
        ...
        showError(
          `No items founds for <span class="fw-bolder fst-italic">${searchData}</span>.`,
          messageContainer
        );
      }
    });
  } else {
    showError("Search Query Can't Be Empty!", messageContainer);
  }
});
